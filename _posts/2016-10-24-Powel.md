...
Currently, the architecture is delivered as a set of individually hosted [Web Apps in Azure App Service](https://azure.microsoft.com/en-us/services/app-service/web/), but is flexible enough to be built into [Azure Service Fabric](https://azure.microsoft.com/en-us/services/service-fabric/) at a later stage if cost becomes an issue. Each web API can be thought of as a component in Service Fabric and deployed separately. Today, the team delivers each web API to a hosted Web App in Azure.

### Domain-Driven Design structure

We organized the code in a [Domain-Driven Design (DDD)](https://en.wikipedia.org/wiki/Domain-driven_design) architecture, focusing on
the entities in the *SmartWater* solution, and the objects that operate upon them. Thus, each component is built up as a collection of class libraries that represent the different layers of execution: presentation, business, data, as well as the domain that is common to all. This made a clean *[separation of concerns](https://en.wikipedia.org/wiki/Separation_of_concerns)* and organized the projects cleanly, allowing different developers to do work in separate levels of the same feature.
...
To provide a satisfying degree of test automation, we adopted the [SOLID principles of object-oriented design](https://en.wikipedia.org/wiki/SOLID_(object-oriented_design). This was mainly done to produce code that can be easily tested in isolation.
...
- [Dependency Inversion Principle](https://en.wikipedia.org/wiki/Dependency_inversion_principle): Essentially, the use of “new()” is banned. All depencencies are resolved using an IoC Container. Otherwise, test-driven approaches are impossible.
...
A base-class for unit testing make dependencies of the class under tested automatically mocked, so that the hassle of unit tests goes away. The base class uses the NuGet package [StructureMap.AutoMocking.Moq](https://www.nuget.org/packages/structuremap.automocking.moq) to achieve this:
...
Test automation ensures the logic of the application is always verified during builds. Another nice NuGet package named [Should](https://www.nuget.org/packages/Should/) gave us a natural language form of writing assertions that everyone instantly adopted; using `Should` makes the assertions more readable:
...
Even though code-coverage doesn’t uncover any quality issues, it was used in the *SmartWater* project as an indicator of potentially problematic code. Libaries dealing with IO have a very limited set of unit tests. 
...
For all of this to happen, the developers adopted modern styles of software development, forming the basis of a healthy codebase that is easy to maintain and easy to understand for new developers joining the team. The SOLID form, combined with DDD and TDD, sets the stage 
for a *SmartWater* solution that can grow in complexity but never drop in its simplicity. 
...
- [Full vertical example of the *SmartWater.SettingsApi*](https://github.com/digitaldias/SmartWater.SettingsApi): contains many of the principles discussed in this article; access keys and other secrets have been masked out, but other than that, the code is complete 
