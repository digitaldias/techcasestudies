...
//	fileName = '2016-05-19-template.md';
	...
	postImageWidthThreshold = process.env['POST_IMAGE_THRESHOLD'],
	featuredImageWidthThreshold = process.env['FEATURED_IMAGE_THRESHOLD'];
;
var PostDirectory = "_posts";

if (!postImageWidthThreshold) postImageWidthThreshold = 1500;
if (!featuredImageWidthThreshold) featuredImageWidthThreshold = 500;
	...
	it("Nomber of modified files", function () {
		if (Object.keys(ModifiedFilesArray).length == 0)	{
			...
			for(var filePath in ModifiedFilesArray){
				...
				// Check the type of file being modified
				...
				if (isDocument) {
					describe("Verifiyng Document: " + fileName, function(){
						...
						describe("Content of the MD file", function(){
							...
							describe("Verify broken image links", function(){
								...
								var match = null;
								var imageLinks = [];
								var featuredImageLinnk = null;
								while (match = pattern.exec(contents)) {
									imageLinks.push(match[1]);
								}

								// Check front matter feature image
								var featPattern = /---([ \u00A0\r\n\S]*?)[\r\n]((image: )+)(.*)([ \u00A0\r\n\S]*?)\s*---/
								var newMatch = featPattern.exec(contents);

								if (newMatch && newMatch.length >= 4) {
									featuredImageLinnk = newMatch[4].replace(/"/g, "").replace(/#.*/, "").trim();
									imageLinks.push(featuredImageLinnk);
								}
										...
										if (imageLink.indexOf("(") >= 0){
											...
											if(imageUrl.hostname) {
												...
												it(imageLink + " should exists in the repository", function(){
													var absolutePath;
													// Front matter links are evaluated differently
													if (image === featuredImageLinnk && !(imageLink.startsWith("./") || imageLink.startsWith("/"))) {
								...
								describe("Verify Featured Image dimensions", function(){
									if(featuredImageLinnk) {
										it("Featured  image (" + featuredImageLinnk + ") should be narrower than " + featuredImageWidthThreshold, function(){
											var dimensions = sizeOf(featuredImageLinnk);
											assert.ok(dimensions.width <= featuredImageWidthThreshold, "Featured image width (" + dimensions.width +  ") whould be smaller than " + featuredImageWidthThreshold)
										});
									} else {
										it.skip("Document doesn't have featured image configured");
									}
								});
							});
				...
				if (isImage) {
					describe("Verifiyng Image: " + fileName, function(){
						var dimensions = sizeOf(fileName);
						var imageWidth = dimensions.width;
						it("Image should be narrower than " + postImageWidthThreshold + " pixels", function(){
							assert.ok(imageWidth <= postImageWidthThreshold, "Image width (" + imageWidth +  ") whould be smaller than " + postImageWidthThreshold)
						})
					});
				}
			}
